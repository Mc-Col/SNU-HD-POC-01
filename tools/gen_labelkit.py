...
라벨링 킷 생성 — schema/fields.yaml → readme/labeling_kit.xlsx
  ...
  python tools/gen_labelkit.py             전체 30필드 (MVP 8개는 앞쪽 진한 색)
...
def main():
    ...
    ap.add_argument("--mvp-only", action="store_true", help="MVP 8필드만 (기본은 전체 30필드)")
    ...
    guide = [
        ...
        ("", "  포맷 배분", "tif(스캔) 15건  ·  pdf 8건  ·  xlsx 5건  ·  xls 1건 + xlsm 1건    ※ 실제 코퍼스가 tif 69%"),
        ...
        ("2", "정답값 기입", "문서를 열어 각 필드의 값을 찾아 '정답값' 칸에 적는다. "
                            "표준값으로 적는다 (예: 문서에 Air-to-Open 이라 적혀 있으면 정답값은 FAIL CLOSE)."),
        ...
        ("", "  사양표가 여러 장이면", "파일명의 태그가 단독으로 나오는 페이지를 고른다. 태그 여러 개가 함께 "
                                  "적힌 공통 사양표(예: 10-FV-011/012/013/014)는 고르지 않는다."),
        ("", "", ""),
        ("3", "원문 라벨 기입", "문서에 적혀 있던 '항목명'을 그대로 옮겨 적는다. "
                               "표준명과 같으면 비워도 된다."),
        ("", "  왜 중요한가", "이 값들이 모여 유사표현 사전이 된다. 30필드의 표기 변종을 "
        ...
        ("4", "문서 분류 기입", "datasheet = 정상 데이터시트  /  "
                              "datasheet_embedded = 정비이력 등에 첨부된 경우  /  "
                              "out_of_scope = 대상 아님"),
        ...
        ("", "", ""),
        ("", "어디까지 채우나", "진한 색 8개(MVP)만 채우면 정확도 측정이 된다. 왼쪽부터 배치되어 있다.\n"
                              "흐린 색 22개(· 표시)는 확장용 — 여유가 있을 때 오른쪽으로 이어서 채우면 된다.\n"
